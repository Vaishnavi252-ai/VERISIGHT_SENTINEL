...
def download_pdf(report_id):
    try:
        ...
        pdf.cell(0, 10, f"VeriSight Sentinel Report #{r.id}", ln=True)
        pdf.set_font('Arial', '', 12)
        pdf.ln(5)
        pdf.multi_cell(0, 8, f"Thank you for submitting your report. Here are the details:")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Detection: {d.media_type} - {d.result_label} (Confidence: {d.confidence * 100:.2f}%)")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Platform: {r.platform} | Country: {r.country or 'Unknown'} | Status: {r.status}")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Reporter Email: {r.reporter_email or 'Not provided'}")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Submitted: {r.created_at}")
        pdf.ln(6)
        pdf.multi_cell(0, 8, "User Q&A:")
        for a in r.answers:
            pdf.multi_cell(0, 8, f"- {a.question}: {a.answer}")

        pdf_output = pdf.output(dest='S').encode('latin1')
        from flask import Response
        return Response(pdf_output, mimetype='application/pdf', headers={"Content-Disposition": f"attachment;filename=report_{r.id}.pdf"})
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({'status': 'error', 'error': str(e)}), 500
...
@admin_bp.route('/api/admin/reports/<int:report_id>/send-pdf', methods=['POST'])
def send_pdf_email(report_id):
    try:
        ...
        # Use provided email or fallback to reporter_email
        recipient_email = email or r.reporter_email
        ...
        pdf.set_font('Arial', '', 12)
        pdf.ln(5)
        pdf.multi_cell(0, 8, f"Thank you for your report. Here are the details:")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Detection: {d.media_type} - {d.result_label} (Confidence: {d.confidence * 100:.2f}%)")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Platform: {r.platform} | Country: {r.country} | Status: {r.status}")
        pdf.ln(4)
        pdf.multi_cell(0, 8, f"Submitted: {r.created_at}")
        pdf.ln(6)
        pdf.multi_cell(0, 8, "Your Q&A:")
        for a in r.answers:
            pdf.multi_cell(0, 8, f"- {a.question}: {a.answer}")
        ...
        subject = f"VeriSight Sentinel Report #{r.id} - {r.status}"
        body = f"Dear Reporter,\n\nAttached is your report details for Report #{r.id}.\n\nStatus: {r.status}\n\nThank you for helping combat deepfakes.\n\nBest regards,\nVeriSight Sentinel Team"
        success = send_email_with_attachment(recipient_email, subject, body, pdf_output, f"report_{r.id}.pdf")
